...
Add-Type -AssemblyName system.core # Needed for named pipes
    ...
    if($argList1) { $process1StartInfo.Arguments = $argList1 }
    ...
    if($argList2) { $process2StartInfo.Arguments = $argList2 }
    ...
    try {
        ...
        write-host "Created $procName1 process"
        ...
        if ($process2.HasExited) {
            write-host $procName2 " Exit Code = " $processs2.ExitCode
        } 

        $process1.StandardOutput.Close()
        $process1.WaitForExit()
        if ($process1.HasExited) {
            write-host $procName1 " Exit Code = " $processs1.ExitCode
        }
    } catch {
